...
// Find the big inline script block that starts with MUTE LOCK
// and replace it entirely with a clean, correct version
const scriptStart = html.indexOf('/* ================================================================\r\n   MUTE LOCK');
if (scriptStart === -1) {
    // try unix endings
    const scriptStart2 = html.indexOf('/* ================================================================\n   MUTE LOCK');
    if (scriptStart2 === -1) { console.error('Could not find MUTE LOCK block'); process.exit(1); }
}

// Find the <script> tag before the MUTE LOCK comment
const scriptTagBefore = html.lastIndexOf('<script>', scriptStart !== -1 ? scriptStart : html.indexOf('MUTE LOCK'));
// Find the </script> tag after the FPS MONITOR
const fpsEnd = html.indexOf('})();\n</script>', scriptTagBefore);
const fpsEnd2 = html.indexOf('})();\r\n</script>', scriptTagBefore);
const scriptTagEnd = Math.max(fpsEnd, fpsEnd2);

if (scriptTagBefore === -1 || scriptTagEnd === -1) {
    console.error('Could not find script boundaries. scriptTagBefore:', scriptTagBefore, 'scriptTagEnd:', scriptTagEnd);
    process.exit(1);
}

const endTagLen = html[scriptTagEnd + 7] === '\r' ? '})();\r\n</script>'.length : '})();\n</script>'.length;
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (modal) {
        ...
        // TikTok-style auto-play observer
...
html = html.slice(0, scriptTagBefore) + cleanScript + html.slice(scriptTagEnd + endTagLen);

fs.writeFileSync('public/index.html', html);
console.log('Script block rebuilt cleanly. scriptTagBefore:', scriptTagBefore, 'scriptTagEnd:', scriptTagEnd);
